...
struct SeedDataLoader {
    ...
    private static let incomingTransferPatterns: [(pattern: String, targetCategory: String)] = [
        ("(?i)PAGO\\s+RECIBIDO\\s+DE\\s+STP\\s+POR\\s+ORDEN\\s+DE\\s+TITULAR", "Transfers.To Own Accounts"),
        ("(?i)recibida\\s+(de\\s+la\\s+)?cuenta\\s+4444\\s+BANAMEX", "Transfers.To Own Accounts"),
        ("(?i)PAGO\\s+INTERBANCARIO\\s+PAGO\\s+RECIBIDO\\s+DE.*STP.*TITULAR", "Credit Card Payments.Card Payment Received"),
    ]
