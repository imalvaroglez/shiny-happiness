...
/// start with a "Tarjeta titular 5470...7827" or "Tarjeta adicional ...7801" line.
struct PastedHsbc2NowParser {
    ...
    func parse(_ text: String) -> ParseResult {
        ...
        let titularCard = cardTransactions.first { $0.cardLast4 == header.titularCardLast4 ?? "7827" }?.cardLast4
    ...
    func parseCardSections(lines: [String], fallbackYear: Int) -> ([CardBucket], [UnparsedLine]) {
        ...
        // Order: titular (suffix 7827, then anything else) first.
        let buckets = bucketed
            .map { CardBucket(cardLast4: $0.key, transactions: $0.value) }
            .sorted { (lhs, rhs) -> Bool in
                let lPri = lhs.cardLast4.hasSuffix("7827") ? 0 : 1
                let rPri = rhs.cardLast4.hasSuffix("7827") ? 0 : 1
                return lPri < rPri
            }
