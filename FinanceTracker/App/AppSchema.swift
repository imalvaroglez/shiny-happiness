...
enum FinanceTrackerSchemaV3: VersionedSchema {
    ...
    @Model final class StockPosition {
        var id: UUID
        @Relationship(deleteRule: .nullify) var account: Account?
        ...
        init(id: UUID = UUID(), account: Account? = nil, emisoraSerie: String, name: String? = nil, shares: Decimal, averageCost: Decimal, lastPrice: Decimal? = nil, lastPriceAt: Date? = nil, createdAt: Date = .now) {
            self.id = id; self.account = account; self.emisoraSerie = emisoraSerie; self.name = name
            self.shares = shares; self.averageCost = averageCost; self.lastPrice = lastPrice
            self.lastPriceAt = lastPriceAt; self.createdAt = createdAt
        }
    }
}

enum FinanceTrackerSchemaV4: VersionedSchema {
    static let versionIdentifier = Schema.Version(0, 7, 0)
    static var models: [any PersistentModel.Type] {
        [
            Account.self,
            AccountBalanceSnapshot.self,
    ...
    @Model final class HouseholdPartnerIncomeEstimate {
        var id: UUID
        var monthStart: Date
        var amount: Decimal
        var notes: String?
...
enum FinanceTrackerSchemaV5: VersionedSchema {
    static let versionIdentifier = Schema.Version(0, 8, 0)
    static var models: [any PersistentModel.Type] {
        [
            Account.self,
            AccountBalanceSnapshot.self,
...
enum FinanceTrackerMigrationPlan: SchemaMigrationPlan {
    static var schemas: [any VersionedSchema.Type] {
        [
            FinanceTrackerSchemaV1.self,
            FinanceTrackerSchemaV2.self,
    ...
    private static let migrateV3toV4 = MigrationStage.lightweight(
        fromVersion: FinanceTrackerSchemaV3.self,
        toVersion: FinanceTrackerSchemaV4.self
    )
    ...
    private static let migrateV2toV3 = MigrationStage.custom(
        ...
        didMigrate: { context in
            let accounts = try context.fetch(FetchDescriptor<FinanceTrackerSchemaV3.Account>())
            for account in accounts {
                backfillAccountMetadata(account)
            }

            let transactions = try context.fetch(FetchDescriptor<FinanceTrackerSchemaV3.Transaction>())
    ...
    private static func backfillAccountMetadata(_ account: FinanceTrackerSchemaV3.Account) {
        let kind = account.retirementKindRaw.flatMap(RetirementKind.init(rawValue:)) ?? inferredRetirementKind(account)
    ...
    private static func backfillTransactionSemantics(_ transaction: FinanceTrackerSchemaV3.Transaction) {
        if transaction.movementKindRaw == nil {
            let flow = transaction.flowKindRaw.flatMap(TransactionFlowKind.init(rawValue:)) ?? inferredFlowKind(transaction)
    ...
    private static func inferredFlowKind(_ transaction: FinanceTrackerSchemaV3.Transaction) -> TransactionFlowKind {
        if transaction.isTransfer { return .transfer }
        if transaction.account?.type.isLiability == true {
            if transaction.amount > 0 {
                return transaction.category?.kind == .creditCardPayment ? .payment : .cardCredit
            }
            return .charge
        }
    ...
    private static func inferredRetirementKind(_ account: FinanceTrackerSchemaV3.Account) -> RetirementKind? {
    ...
    private static func inferredTreatmentKind(_ transaction: FinanceTrackerSchemaV3.Transaction) -> TransactionTreatmentKind {
        ...
        switch transaction.account?.retirementKindRaw.flatMap(RetirementKind.init(rawValue:)) ?? inferredRetirementKind(transaction.account!) ?? .other {
        case .ppr:
    ...
    private static func clearlyInvestmentReturn(_ transaction: FinanceTrackerSchemaV3.Transaction) -> Bool {
    ...
    private static func clearlyValuationAdjustment(_ transaction: FinanceTrackerSchemaV3.Transaction) -> Bool {
        let text = "\(transaction.descriptionRaw) \(transaction.category?.name ?? "")".lowercased()
