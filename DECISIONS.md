...
**Decision:** Promotions are bundle JSON definitions (`Domain/Promotions/Catalog/promotions.json` and shared `channel_table.json`) with per-entry validation and visible configuration warnings. The report evaluator returns value results and writes no promotion state. It filters by account and explicit evaluation date, keeps reconciliation evidence distinct from eligible spend, ignores dashboard cash-flow preferences, and reconciles MSI/refunds against valid account history before summing. Only unique merchant identity plus compatible amount can bind a credit; disputes remain outside the firm and visible. Definitions bind by account UUID (nickname is authoring help only); overlaps are warnings by candidate transaction IDs; reward credits remain unassigned. Ambiguity that can change a threshold makes its result provisional. `postedAt` is the only transaction date: statement bounds are the min/max of imported transaction dates, not issuer-cycle dates, so any billing-date relationship remains an explicit assumption.
**Consequence:** The feature adds no schema or persisted promotion state. Its output is a report estimate, not an issuer qualification or statement-cycle determination; correctness against representative account history and runtime performance require separate development validation. Configuration changes require editing JSON and rebuilding. V2 may consider persisted per-transaction overrides, reward confirmation, installment projections, or joint receipt matching. `promo.py` remains available for comparison until the replacement is explicitly validated.
