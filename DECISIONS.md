...
## AD-024: Credit-card promotions are a declarative report layer (bundle JSON + pure evaluator), not persisted state
**Date:** 2026-09-22
**Context:** Tracking issuer promotions (welcome bonuses, capped cashback, periodic thresholds) previously lived only in the `finanzas` skill's `promo.py` CLI over `.ftbackup` exports. The app owns the transactions but had no promotion concept. `Transaction`'s schema is frozen (AD-023) and every new `@Model` costs a schema version + backup surface + specialized data review — disproportionate before real Gold-card data exists.
**Decision:** Promotions are declarative definitions in bundle JSON (`Domain/Promotions/Catalog/promotions.json` + shared `channel_table.json`, Knowledge-loader pattern: fail-soft with visible warnings). `PromotionEvaluator` is a pure @MainActor calculator over live models (AD-022 mold): stages keep reconciliation evidence distinct from eligible spend (an MSI reversal categorized as Credit Card Payments still reconciles), never gate on `countsAsRegularExpense` (dashboard preferences must not move progress), reconcile MSI conversions and refunds over the account's full history up to an explicit evaluation date (neutralize the original only on unambiguous correspondence; disputes leave the firm with a declared direction; refunds apply to the original charge capped at its amount), bind definitions by account UUID only (nickname is authoring aid; unbound = not calculable), detect overlaps by crossing candidate transaction IDs, list reward-credit candidates without assignment, and suspend "umbral alcanzado según registros" when a pending ambiguity could change the outcome. postedAt is the only date (Statement.periodStart/End are min/max of imported transaction dates, not the issuer cycle); billing-date rules are declared assumptions.
**Consequence:** Opening a card account shows promotion progress in seconds with honest uncertainty (not calculable / estimate-under-assumptions / review bands), zero persistence or schema risk, and promo.py's MSI double-count bug is fixed at the root. Editing promotions is editing JSON + rebuild (owner-developer). Deferred to V2 with pre-justified ledger: persisted per-tx overrides, reward confirmations, installment projections, receipt matcher (joint per-account resolution). promo.py stays operational until the owner validates the replacement with real data.
