...
struct SPEIDestinationRulesTests {
    ...
    private func makeRules() -> [CategoryRule] {
        ...
        return [
            ...
            CategoryRule(patternRegex: "(?i)SPEI enviada a BBVA 7777", category: toOwn, priority: 87),
    ...
    func testSpeiTo2now() {
        let rules = makeRules()
        let tx = makeTransaction(description: "SPEI enviada a 2now 3333, HSBC, 2000010100000000000000000")
    ...
    func testSpeiToPriority() {
        let rules = makeRules()
        let tx = makeTransaction(description: "SPEI enviada a Priority4444, BANAMEX, 20000102001")
        _ = Categorizer.categorize(transactions: [tx], rules: rules)
    ...
    func testSpeiToNu() {
        let rules = makeRules()
        let tx = makeTransaction(description: "SPEI enviada a Nu 5555, NU MEXICO, 20000101001")
        _ = Categorizer.categorize(transactions: [tx], rules: rules)
    ...
    func testSpeiToVolaris() {
        let rules = makeRules()
        let tx = makeTransaction(description: "SPEI enviada a INVEX Volaris 0 6666, INVEX, 20000103001")
        _ = Categorizer.categorize(transactions: [tx], rules: rules)
    ...
    func testTransferSubcategoriesExcluded() async throws {
        let schema = Schema([Account.self, Transaction.self, Statement.self, FinanceTracker.Category.self, CategoryRule.self])
