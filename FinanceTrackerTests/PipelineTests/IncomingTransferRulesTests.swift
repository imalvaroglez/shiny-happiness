...
struct IncomingTransferRulesTests {
    ...
    private func makeSeedRules() -> [CategoryRule] {
        ...
        return [
            makeRule(pattern: "(?i)PAGO\\s+RECIBIDO\\s+DE\\s+STP\\s+POR\\s+ORDEN\\s+DE\\s+ALVARO", categoryName: "To Own Accounts", categoryKind: .transfer, priority: 110),
            makeRule(pattern: "(?i)recibida\\s+(de\\s+la\\s+)?cuenta\\s+0728\\s+BANAMEX", categoryName: "To Own Accounts", categoryKind: .transfer, priority: 110),
            makeRule(pattern: "(?i)PAGO\\s+INTERBANCARIO\\s+PAGO\\s+RECIBIDO\\s+DE.*STP.*ALVARO", categoryName: "Card Payment Received", categoryKind: .creditCardPayment, priority: 110),
        ]
    }
    ...
    func stpIncomingOwnTransfer() {
        let tx = makeTransaction(description: "PAGO RECIBIDO DE STP POR ORDEN DE ALVARO JOSUE Transferencia 20260515")
    ...
    @Test("Incoming from own Banamex 0728 categorized as transfer")
    func incomingFromOwnBanamex() {
        let tx = makeTransaction(description: "Transferencia recibida de la cuenta 0728 BANAMEX, ALVARO JOSUE, 20260515")
        let rules = makeSeedRules() + makeLearnedIncomeRules()
    ...
    func creditCardPaymentReceived() {
        let tx = makeTransaction(description: "PAGO INTERBANCARIO PAGO RECIBIDO DE: STP/SIST TRANSF Y PAGOS ALVARO JOSUE 20260515")
        let rules = makeSeedRules() + makeLearnedIncomeRules()
    ...
    func priority110BeatsPriority90() {
        let tx = makeTransaction(description: "PAGO RECIBIDO DE STP POR ORDEN DE ALVARO JOSUE MARIN")
